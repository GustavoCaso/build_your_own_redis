...
{
  // wake up by poll, update idle timer
...
{
  // the extra 1000us is for the ms resolution of pool()
...
int main()
{
  ...
  while (true)
  {
    // prpeare the arguments for poll()
    poll_args.clear();
    ...
    for (size_t i = 1; i < poll_args.size(); i++)
    {
      if (poll_args[i].revents)
      {
        ...
        {
          // client closed normally, or something bad hapenned.
